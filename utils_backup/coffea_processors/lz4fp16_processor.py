...
class LZ4FP16Processing(DataPreprocessing_BaseClass):
    ...
    def saveOutput(
        ...
        arr = np.concatenate([
            global_arr.view((global_arr.dtype[0], len(global_arr.dtype.names))).astype(np.float16).reshape(n_jet, -1),
            np.swapaxes(cpf_arr.view((cpf_arr.dtype[0], len(cpf_arr.dtype.names))).astype(np.float16), 1,2).reshape(n_jet, -1),
            np.swapaxes(npf_arr.view((npf_arr.dtype[0], len(npf_arr.dtype.names))).astype(np.float16), 1,2).reshape(n_jet, -1),
            np.swapaxes(vtx_arr.view((vtx_arr.dtype[0], len(vtx_arr.dtype.names))).astype(np.float16), 1,2).reshape(n_jet, -1),
            process.astype(np.float16).reshape(n_jet, -1),
            truth.view((truth.dtype[0], len(truth.dtype.names))).astype(np.float16).reshape(n_jet, -1)
        ]
        ,axis=1)
